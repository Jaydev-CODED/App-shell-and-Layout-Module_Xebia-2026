...
export default function Sidebar({ isCollapsed, onToggleCollapse, activeRoute }) {
  const menuItems = [
    ...
    { id: 'export', name: 'Export Logs', icon: Share2, path: '/export' },
    { id: 'compliance', name: 'Compliance', icon: ShieldCheck, path: '/compliance' },
    { id: 'activity', name: 'Activity Timeline', icon: History, path: '/activity' },
    { id: 'settings', name: 'Settings', icon: Settings, path: '/settings/configuration' },
  ];
  ...
  return (
    ...
    >
      <nav className="flex flex-col gap-2" aria-label="Sidebar navigation">
        {menuItems.map((item) => {
          // Normalize active route detection
          const isActive = activeRoute === item.id || 
            (item.id === 'settings' && activeRoute.startsWith('settings')) ||
            (item.id === 'audit' && activeRoute.startsWith('audit-detail'));
